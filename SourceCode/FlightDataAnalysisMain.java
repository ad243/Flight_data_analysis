...
//	private static final String TEMP_OUTPUT_PATH1 = "/temp_output11";
//	private static final String TEMP_OUTPUT_PATH2 = "/temp_output22";

	private static final String TEMP_OUTPUT_PATH1 = "/Users/anand/NJIT/Big Data/repo/BigData-FlightDataAnalysis/temp_output1";
	private static final String TEMP_OUTPUT_PATH2 = "/Users/anand/NJIT/Big Data/repo/BigData-FlightDataAnalysis/temp_output2";
	...
	public int run(String[] arg0) throws Exception {
		try {
			...
			String inputFile = "2008.csv";
			String outputFile = "output";
			FileInputFormat.setInputPaths(job, new Path(inputFile));


			// FileSystem fs = FileSystem.get(getConf());
			if (fs.exists(new Path(outputFile + "_FlightCancellationMostCommonReason"))) {
				fs.delete(new Path(outputFile + "_FlightCancellationMostCommonReason"), true);
			}
			...
			FileOutputFormat.setOutputPath(job, new Path(inputFile + " v"));
			...
			if (fs.exists(new Path(outputFile + "_AirportTaxiInTime_Max"))) {
				fs.delete(new Path(outputFile + "_AirportTaxiInTime_Max"), true);
			}
			FileOutputFormat.setOutputPath(job3, new Path(outputFile + "_AirportTaxiInTime_Max"));
			...
			if (fs.exists(new Path(outputFile + "_AirportTaxiInTime_Min"))) {
				fs.delete(new Path(outputFile + "_AirportTaxiInTime_Min"), true);
			}
			FileOutputFormat.setOutputPath(job4, new Path(outputFile + "_AirportTaxiInTime_Min"));
			...
			if (fs.exists(new Path(outputFile + "_AirportTaxiOutTime_Max"))) {
				fs.delete(new Path(outputFile + "_AirportTaxiOutTime_Max"), true);
			}
			FileOutputFormat.setOutputPath(job6, new Path(outputFile + "_AirportTaxiOutTime_Max"));
			...
			if (fs.exists(new Path(outputFile + "_AirportTaxiOutTime_Min"))) {
				fs.delete(new Path(outputFile + "_AirportTaxiOutTime_Min"), true);
			}
			FileOutputFormat.setOutputPath(job7, new Path(outputFile + "_AirportTaxiOutTime_Min"));
			...
			FileInputFormat.setInputPaths(job8, new Path(inputFile));
			if (fs.exists(new Path(outputFile + "_AirlinesBeingOnSchedule"))) {
				fs.delete(new Path(outputFile + "_AirlinesBeingOnSchedule"), true);
			}

			FileOutputFormat.setOutputPath(job8, new Path(outputFile + "_AirlinesBeingOnSchedule"));
			// TODO Auto-generated method stub
